...
BEGIN
  ...
  IF v_last_rpe < 7 THEN
    ...
    v_reasoning := 'RPE war niedrig (' || v_last_rpe || ') - Gewicht um ' || v_increment || 'kg erhoehen';
  ELSIF v_last_rpe <= 8.5 AND v_last_reps >= p_target_reps THEN
    v_recommended := v_last_weight + v_increment;
    v_reasoning := 'Ziel-Reps (' || p_target_reps || ') erreicht bei gutem RPE - Gewicht erhoehen';
